...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_theme_color_values(self):
        return self.env["web_editor.assets"].get_color_variables_values(
            self.COLOR_ASSET_THEME_URL, self.COLOR_BUNDLE_THEME_NAME, self.THEME_COLOR_FIELDS
        )
    ...
    def _replace_theme_color_values(self):
        variables = [
            {"name": field, "value": self[f"theme_{field}"]} for field in self.THEME_COLOR_FIELDS
        ]
