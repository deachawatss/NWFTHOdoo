...
class ProductSearch(models.TransientModel):
    ...
    action_id = fields.Many2one(
        ...
        string="Action",
        required=True,
        domain=[("res_model", "=", "product.template"), ("type", "=", "ir.actions.act_window")],
    ...
    @api.depends("search_value", "value_split_operator", "search_operator", "search_field")
            ...
            if not search_value:
                ...
                for part in search_parts:
                    search_domain = expression.OR(
                        [search_domain, [(record.search_field, record.search_operator, part)]]
                    )
    ...
    def action_search_products(self):
        ...
        if self.search_domain:
            action["domain"] = ast.literal_eval(self.search_domain)
        return action
